...
describe("financial conversion boundary coverage", () => {
  ...
  it("rejects zero valuation", () => {
    expect(() => calculatePricePerShare("0", "10")).toThrow();
  });
  it("rejects zero capitalization", () => {
    expect(() => calculatePricePerShare("10", "0")).toThrow();
  });
  it("rejects negative valuation", () => {
    expect(() => calculatePricePerShare("-1", "10")).toThrow();
  });
  it("rejects negative capitalization", () => {
    expect(() => calculatePricePerShare("10", "-1")).toThrow();
  });
  ...
  it("selects lower discount with cap", () => {
    ...
    expect(result.conversionPrice).toBe("1.5000000000");
  });
  ...
  it("rejects negative discount", () => {
    expect(() => convertPostMoneySafe({ type: "POST_MONEY_DISCOUNT", purchaseAmount: "1", discountRate: -1, mfnEnabled: false, proRataEnabled: false }, baseContext)).toThrow();
  });
  ...
  it("rejects negative principal", () => {
    expect(() => calculateNoteAccrual("-100", 10, new Date("2025-01-01"), new Date("2025-02-01"))).toThrow();
  });
  it("rejects negative interest", () => {
    expect(() => calculateNoteAccrual("100", -1, new Date("2025-01-01"), new Date("2025-02-01"))).toThrow();
  });
  ...
  it("converts note with discount", () => {
    const result = convertNote({ principal: "100000", interestRate: 8, maturityDate: "2027-01-01", discountRate: 20, qualifiedFinancingMin: "1000000", maturityTreatment: "CONVERT_AT_DISCOUNT" }, new Date("2025-01-01"), new Date("2026-01-01"), { pricePerShare: "1", companyCapitalization: "10000000" });
    expect(result.appliedMechanism).toBe("DISCOUNT");
  });
  ...
  it("rejects negative pre-money", () => {
    expect(() => calculateDilution({ preMoneyValuation: "-1", investmentAmount: "10", optionPoolPostMoneyPercent: 10 })).toThrow();
  });
  it("rejects negative investment", () => {
    expect(() => calculateDilution({ preMoneyValuation: "100", investmentAmount: "-1", optionPoolPostMoneyPercent: 10 })).toThrow();
  });
  it("rejects option pool above one hundred", () => {
    expect(() => calculateDilution({ preMoneyValuation: "100", investmentAmount: "10", optionPoolPostMoneyPercent: 101 })).toThrow();
  });
  it("rejects option pool below zero", () => {
    expect(() => calculateDilution({ preMoneyValuation: "100", investmentAmount: "10", optionPoolPostMoneyPercent: -1 })).toThrow();
  });
